...
export default function Sidebar(props) {
  ...
  const notifySidebarChange = (open) => {
    // Use props callback if provided
    if (props.onToggle) {
      props.onToggle(open);
    }

    // Also dispatch an event for components that aren't directly parent/child
  ...
  return (
    <aside
      ...
      } z-30 h-full border-r shadow-lg ${sidebarClass()} hidden md:block`}
      style={{
        width: width(),
        transform: isOpen()
          ? "translateX(0)"
