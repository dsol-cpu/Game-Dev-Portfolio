...
export default function Sidebar(props) {
  ...
  // Game-inspired theme classes with memoized values
  const sidebarClass = () =>
    isDark()
  ...
  createEffect(() => {
    setIsOpen(!props.isMobile);
    notifySidebarChange(!props.isMobile);
  });
  ...
  const triggerShipNavigation = (islandIndex) => {
    // Find the ship navigation function in the window scope
    if (
  ...
  const navigateToSection = (sectionId, islandIndex, event) => {
    ...
    // Set target island but also directly trigger the navigation
    setTargetIsland(islandIndex);

    // Properly set navigation state
    setIsNavigating(true);
    setNavigationProgress(0);

    // Direct trigger of ship navigation - important fix
