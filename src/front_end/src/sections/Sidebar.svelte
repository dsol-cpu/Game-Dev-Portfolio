<script>
	...
	function smoothScroll(sectionId, event) {
		event.preventDefault(); // Prevent default behavior that adds # to URL
...
{#if toggleButton}
	<button
		class="sidebar-toggle fixed top-16 {position === 'left'
			? 'left-4'
			: 'right-4'} z-40 rounded-full bg-gradient-to-br from-blue-600 to-teal-500 p-2 text-white shadow-lg hover:from-blue-500 hover:to-teal-400 md:hidden"
...
<aside
	class="sidebar fixed top-0 {position === 'left'
		? 'left-0'
		: 'right-0'} z-30 h-full border-r shadow-lg {sidebarClass}"
	style="width: {width}; transform: translateX({isOpen || window.innerWidth >= 768
		? '0'
		: position === 'left'
			? '-100%'
		...
		<nav class="flex-1 overflow-y-auto px-4">
			<ul class="space-y-2 py-4">
				{#each ['home', 'experience', 'projects'] as section}
					<li>
						<button
							class="flex w-full items-center rounded-lg px-4 py-3 font-medium transition-all duration-200 {activeSection ===
							section
								? activeClass
								: itemClass}"
							on:click={(e) => smoothScroll(section, e)}
						>
